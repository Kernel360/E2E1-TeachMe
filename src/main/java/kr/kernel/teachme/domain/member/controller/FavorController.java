...
public class FavorController {
	...
	public String getFavorList(Model model,
		...
		List<Lecture> favorLectures = memberFavorService.getFavorLectureList(member);
	...
	@PostMapping("/add")
	public ResponseEntity<FavorResponse> addFavorLecture(@RequestBody FavorRequest request, @CookieValue(name = JwtProperties.COOKIE_NAME, required = true) String token) {
	...
	public ResponseEntity<FavorResponse> deleteFavorLecture(@RequestBody FavorRequest request,
								   @CookieValue(name = JwtProperties.COOKIE_NAME, required = true) String token) {
		FavorResponse response = new FavorResponse();
		try {
